...
def generate_numeric_rep(val):
    """High-speed numeric generation preserving length and precision"""
    s = str(val).strip()
    is_neg = s.startswith('-')
    abs_s = s.lstrip('-')
    ...
    # Generate digits as strings first to avoid floating point drift
...
def generate_string_rep(val):
    """High-speed string replacement preserving format"""
    s = str(val).strip()
...
def scan_file(file_info):
    """Parallel worker to find unique values using pandas for XLSX conversion"""
    ...
    try:
        ...
        # --- MODIFICATION: Removed xlsx2csv, using pd.read_excel ---
        if file_name.endswith('.xlsx'):
            sheet = settings.get('sheet', 0) if isinstance(settings, dict) else 0
            # Read the excel sheet and save as temp CSV to allow chunking
        ...
        # Standard chunked reading for both original CSVs and temporary converted CSVs
        reader = pd.read_csv(working_path, chunksize=CHUNK_SIZE, usecols=cols, low_memory=False)
        for chunk in reader:
            for col in cols:
                if col in chunk.columns:
                    # Clean unique values to prevent duplicates (e.g., 45.0 vs 45)
...
def generate_mapping():
    ...
    # --- GLOBAL AGGREGATION ---
    # Combine all unique values from all files into one master set
    global_uniques = set()
